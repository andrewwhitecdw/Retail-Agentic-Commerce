...
if $MILVUS_READY; then
    ...
    if env "${AGENT_ENV_ARGS[@]}" "$AGENTS_VENV/bin/python" "$AGENTS_DIR/scripts/seed_milvus.py" > "$LOG_DIR/milvus-seeder.log" 2>&1; then
...
start_agent() {
    ...
    cd "$AGENTS_DIR"
    env "${AGENT_ENV_ARGS[@]}" "$AGENTS_VENV/bin/nat" serve --config_file "configs/$config" --port "$port" > "$logfile" 2>&1 &
    local pid=$!
    echo "$pid:$name" >> "$PID_FILE"
    SVC_NAMES+=("$name")
    SVC_PORTS+=("$port")
